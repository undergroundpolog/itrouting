 ...
 */
package com.intraffic.org.neo4j.graphalgo.impl.path;
...
public class AStar implements PathFinder<WeightedPath>
{
    ...
    public WeightedPath findSinglePath( Node start, Node end, Object params)
    {
        lastMetadata = new Metadata();
        Doer doer = new Doer( start, end , params);
        while ( doer.hasNext() )
        {
            Node node = doer.next();
    ...
    {
        WeightedPath path = findSinglePath( node, end , null);
    ...
    private class Doer extends PrefetchingIterator<Node> implements Path
    {
        ...
        Doer( Node start, Node end , Object params)
        {
            this.start = start;
            this.end = end;
            this.params = params;
        ...
        private void expand()
        {
            for ( Relationship rel : expander.expand( this, BranchState.NO_STATE ) )
            {
                ...
                double tentativeGScore = lastNodeData.wayLength +
                        lengthEvaluator.getCost( rel, Direction.OUTGOING , this.params);
